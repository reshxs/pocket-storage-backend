...
class TypedPaginator(tp.Generic[_ST]):
    ...
    def get_response(
        ...
        **model_kwargs: tp.Any,
        ...
        orphans = 1
        items = [
            self.schema.from_model(o, *model_args, **model_kwargs)
            for o in self.query[bottom : top + orphans]
        ]
...
class TypedPaginatorWithCustomParams(TypedPaginator):
    def __init__(
        self, schema: tp.Type[_ST], query: QuerySet, paginated_response: tp.Any
    ):
        assert isinstance(
    ...
    def get_response(
        ...
        *model_args: tp.Iterable[tp.Any],
        ...
        orphans = 1
        items = [
            self.schema.from_model(o, *model_args)
            for o in self.query[bottom : top + orphans]
        ]
